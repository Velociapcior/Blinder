...
FR41: Epic 9 — PostHog reveal event tracking
...
Developers can run the full Blinder stack locally via Docker Compose from a single command, with a scaffolded ASP.NET Core backend, Expo mobile project, Nginx reverse proxy with SignalR WebSocket headers, PostHog analytics container, and all environment conventions established. This foundation enables every subsequent epic.
...
An operator can view a near-real-time gender ratio dashboard. Users can request personal data export and data deletion (GDPR rights). All moderation actions are recorded in tamper-evident audit logs retained for 2 years. PostHog tracks reveal initiation, confirmation, and abandonment as distinct events.
...
**Then** four containers start successfully: `api` (ASP.NET Core), `db` (postgis/postgis), `nginx` (reverse proxy), `posthog` (analytics)
...
**Then** `expo-router`, `expo-notifications`, `expo-image-picker`, `expo-secure-store`, `@microsoft/signalr`, `nativewind`, `posthog-react-native` are all installed
...
### Story 6.6: PostHog Reveal Funnel Event Tracking

As a product analyst,
I want PostHog to capture every reveal funnel event as distinct trackable events,
So that we can measure the north star metric (>40% of active users complete a mutual reveal within 60 days).

**Acceptance Criteria:**

**Given** a user taps "I'm ready" (`RevealPrompt.tsx`)
**When** the reveal readiness flag is set
**Then** a PostHog event `reveal_initiation` is captured with properties: `conversation_id` (hashed), `message_count_at_initiation`, `user_tier` (free/premium)

**Given** mutual reveal is confirmed (both flags true)
**When** `RevealStateUpdated` event fires
**Then** a PostHog event `reveal_confirmed` is captured with properties: `conversation_id` (hashed), `time_to_mutual_reveal_days`, `message_count_at_reveal`

**Given** one user sets their flag but the other user never does (conversation eventually archived)
**When** the conversation is closed
**Then** a PostHog event `reveal_abandoned` is captured with properties: `conversation_id` (hashed), `initiated_by_user_tier`, `days_since_initiation`

**Given** PostHog is configured
**When** event capture is reviewed
**Then** `<1%` event loss rate (NFR29); no PII (user IDs, names, email) appears in any PostHog event property — only hashed conversation IDs
...
An operator can view a near-real-time gender ratio dashboard. Users can request data export and deletion. All moderation actions are in tamper-evident audit logs. PostHog tracks all reveal funnel events.

### Story 9.1: PostHog Self-Hosted Setup and Core Analytics Instrumentation

As a product analyst,
I want PostHog running self-hosted on the EU-region VPS and capturing key user journey events,
So that all analytics data stays within the EU and I can measure product health from day one.

**Acceptance Criteria:**

**Given** `docker-compose.yml` is reviewed
**When** the PostHog service is inspected
**Then** PostHog runs as a container on the same EU-region VPS; data does NOT transit non-EU cloud regions (NFR13); PostHog is accessible at the internal Docker network only — not exposed publicly

**Given** `posthog-react-native` is installed in the mobile app
**When** the PostHog client is initialised
**Then** it is initialised once at app startup with the self-hosted PostHog API endpoint; `optOut: true` is set during Jest tests to prevent test events polluting analytics data

**Given** the following user actions occur
**When** PostHog event capture is verified
**Then** the following events are captured: `user_registered`, `onboarding_completed`, `match_assigned`, `conversation_started`, `message_sent` (count only, no body), `reveal_initiation`, `reveal_confirmed`, `reveal_abandoned`

**Given** any PostHog event is captured
**When** the event properties are reviewed
**Then** no PII is present in any event property — user IDs are hashed before being used as PostHog distinct IDs; message bodies are never captured; email addresses are never sent to PostHog

---

### Story 9.2: Gender Ratio Dashboard
...
### Story 9.3: Tamper-Evident Moderation Audit Log
...
### Story 9.4: GDPR Personal Data Export
...
### Story 9.5: GDPR Right to Erasure
